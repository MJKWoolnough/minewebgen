...
func add(c dom.Element) {
	...
	u.AddEventListener("click", false, func(dom.Event) { upload(c) })
	d := xjs.CreateElement("input")
	d.SetAttribute("value", "Download")
	d.SetAttribute("type", "button")
	d.AddEventListener("click", false, func(dom.Event) { download(c) })
	g := xjs.CreateElement("input")
	g.SetAttribute("value", "Generate")
	g.SetAttribute("type", "button")
	g.AddEventListener("click", false, func(dom.Event) { generate(c) })
	c.AppendChild(u)
	c.AppendChild(d)
	c.AppendChild(g)
}
